...
export function signTableToken(tableId: string | number, ttlSeconds = 60 * 60 * 24 * 365) {
...
export function verifyTableToken(token: string) {
  try {
    ...
    if (isNaN(expiresAt) || expiresAt < Math.floor(Date.now() / 1000)) return { valid: false, expired: true }
